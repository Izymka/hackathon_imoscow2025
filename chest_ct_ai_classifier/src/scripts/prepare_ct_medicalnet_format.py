...
Сохраняет совместимость с оригинальной подготовкой данных MedicalNet.
"""
...
# -------------------------------
# Трансформации MONAI (оригинальные для MedicalNet)
# -------------------------------
def get_transforms():
    """Трансформации с изотропным ресемплингом до 1x1x1 мм"""
    return Compose([
        EnsureChannelFirst(channel_dim="no_channel"),
        # Изотропный ресемплинг до 1x1x1 мм
        Spacing(pixdim=TARGET_SPACING, mode="bilinear", padding_mode="border"),
        # Обрезка фона (HU > -1000 для КТ)
        CropForeground(select_fn=lambda x: x > -1000, channel_indices=0, margin=5),
        # Масштабирование HU диапазона [-1000, 600] к [0, 1]
        ScaleIntensityRange(
            a_min=HU_MIN,
            a_max=HU_MAX,
            b_min=0.0,
        ...
        # Адаптивное изменение размера до целевого
        Lambda(lambda x: F.interpolate(
            x.unsqueeze(0).unsqueeze(0),  # [D, H, W] → [1, 1, D, H, W]
            size=TARGET_OUTPUT_SHAPE,
            mode='trilinear',
            align_corners=False
        ).squeeze(0).squeeze(0)),  # [1, 1, D, H, W] → [D, H, W]
        ToTensor()
    ])
...
def process_single_patient(patient_dir: Path, output_dir: Path, verbose: bool = False) -> dict:
    ...
    try:
        ...
        # Добавляем размерность батча: [D, H, W] → [1, 1, D, H, W]
        final_tensor = transformed_tensor.unsqueeze(0).unsqueeze(0)
...
def main():
    ...
    logger.info(f"✅ MedicalNet-compatible preprocessing")
